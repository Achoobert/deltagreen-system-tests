  ...
  quench.registerBatch(
    ...
    (context) => {
      ...
      describe('Exhaustion, rest, stimulants', function () {
        ...
        let actor
        let syncExhaustionEffect
        let clearStimulantEffects

        before(async function () {
          const stimulant = await dgImport(
            '/systems/deltagreen/module/active-effect/runtime/stimulant-effect.js'
          )
          const exhaustion = await dgImport(
            '/systems/deltagreen/module/active-effect/runtime/exhaustion-effect.js'
          )
          syncExhaustionEffect = exhaustion.syncExhaustionEffect
          clearStimulantEffects = stimulant.clearStimulantEffects

          actor = await createTestAgent('physical-shared')
